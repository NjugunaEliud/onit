...
const SettingBoxes = () => {
  const [adminData, setAdminData] = useState<{ name?: string; email?: string; utype?:string;phone?:string;}>({}); 
  useEffect(() => {
    const adminDatas = window.localStorage.getItem("AdminData");
    if (adminDatas) {
      try {
        const parsedData = JSON.parse(adminDatas);
