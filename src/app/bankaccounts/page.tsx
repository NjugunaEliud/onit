...
const BankDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const response = axios.get('https://us-central1-onit-439704.cloudfunctions.net/banks')
      .then(response => {
        console.log("Response", response.data.payload)
        setBanks(response.data.payload)
      })
      .catch(error => [
        Alert(error)
      ])
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-gray-100 dark:bg-gray-800">
                      <th className="px-4 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">Bank ID</th>
                      <th className="px-4 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">Name</th>
                      <th className="px-4 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400">Bank Code</th>
                    </tr>
                  </thead>
                  <tbody>
                    {banks.map((item, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800'}>
                        <td className="px-4 py-6 text-sm font-medium text-gray-900 dark:text-white">
                          {item.bankId}
                        </td>
                        <td className="px-4 py-6 text-sm font-medium text-gray-900 dark:text-white">
                          {item.name}
